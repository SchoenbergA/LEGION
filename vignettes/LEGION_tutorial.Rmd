...
So now with our 7 ALs containing Indice Stack (cleaned from higly correlating layers) we can go on for computing filters.

### Filter Usage

LEGION delivers (atm) 9 differet filters: Sum, Min, Max, Mean, Sd, Modal and Sobal (combined and single horizontal/vertical). Each filter uses a moving window and requires odd values (eg 3x3). The only limitation for the filter sizes is the relation to the x and y lenght of the input data. So for a 100x100 pixel image it makes no sense to filter with a 51x51 moving window (I would even say more than 15 would be waste of time). Here we encounter a major Problem in the "let the maschine do the job" idea, cause there is no "rule" to define the "best" filter or Moving window. But with the dropping of highly correlating ALs we just saved a lot of time. 
...
```
We see again about less than half of the ALs correlate (4/9) even with THvalue of 0.9. For our final Stack this would mean to reduce it from 224 ALs to 140 ALs.

Unlike with the indices we now select the remaining filter types by hand to compute tham for the hole Indice Stack. But NOTE: like with the indices if you want to keep a layer which has been detected to correlate feel free to keep it (because reasons)
...
justcheckFILTER <- LEGION::detct_RstCor(sumMW,0.8)
...
justcheckFILTER <- LEGION::detct_RstCor(sdMW,0.9)
...
justcheckFINAL <- LEGION::detct_RstCor(finalStk,0.8)
...
If we use finally the detect cor on our final Stack exacly half of the layers are dropped. If we look on the Cor plot we can se some relationsships that within an Index some of the filters correlate.

But finally our goal was to check for correlation between the availbe indices and filter types. So to keep those ALs or to drop them is NOT content of this Tutorial or LEGION

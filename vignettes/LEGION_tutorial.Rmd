...
For several spatial environmental tasks like maschinelearing based classifications or ordination and cluster analysis, artifical layers (further called 'ALs') are needed as predictors. These predictors should deliver as much information as possible without overfitting. The decision which AL should be computed and used can lead frustration and 'overthinking'. In science we can NOT just say we will use "this index and that filter". We should (or better: it would be desireable :-) to) comprehensibly justify our desicions. Best (I would say) with automated statisical approaches.

We could just do a "brute force" approach. LEGION delivers up 15 diffenrent indices (if NIR is available) and 9 different filter. If we would compute all indices and use all filter with MovingWindows of (just) 3,5,7,9 we would end up with 15 * 9 * 4 = 540 ALs. 

One major problem with ALs is the risk of overfitting by highly correlating layers. Furthermore maschine learing/ordination and cluster analysis with a RasterStack containing higly correlating ALs could take unnesserarily long time to proceed. So instead of thinking too much about which AL could be useful let the machine do the job :)
...
In general LEGION supports two major ways to compute a RasterStack with ALs. The first is to compute all possible indices and use all availbe filter functions on those indices. The only desicion would be the amount of the different MovingWindows for the filters. This desicion should be based on the extent of the Input RasterLayer. Finally the RasterStack should be cleaned up from correlating ALs. In this case no information would be lost but this could take a lot of time to comtue the RasterStack. If time does not play a big role (eg overnight usage) this way is recommended for best results.

If time is essential or with big input data it could be helpful to reduce the amount of ALs. In this case the correlations should be detected before further processing BUT it is possible to lose some informations.

This tutorial will lead you throught the 'fast' process of computing a RasterStack with several ALs with LEGION. Hereby we will start with just one input RasterLayer and 'form up a LEGION of ALs': the output RasterStack. Furthermore we will make sure that the output RasterStack is clean from highly correlating layers. (for the 'brute force' approach see respectif chapter)

## Getting started

### First of all: the input data

LEGION is created to create 'many from one', what means that it generates many ALs from just one input data source. So it is important to know where the data comes from. Usually it is some sort of aerial/satellite image. LEGION is made to support from drone derived orthoimages, over high resolution airborne orthoimages up to multispectral satellite imagery (like Sentinal 2a). However it is essential to know the bandorder. The workflow of LEGION is primalily based on RGB and additionally supports indices with Near-Infrared (NIR). For multispectral satellite data the RGB+NIR bands can be used, but (untill now) LEGION doest NOT support indices which use any other bands than RGB+NIR.

### Example data

The attached example dataset is a RasterStack with the spectral bands red, green and blue from an high resolution (0.15m) orthoimage. Further the NIR band has been utilized from an IRC (IR Composite) orthoimage with the same resolution. Note that the bands are sorted in the order 1=blue, 2=green, 3=red, nir=4. The bands are sorted like their band-order in satellite data. Furtzher note: in unknown datasets RGB could mean band-order 3,2,1 or 1,2,3.

The example data depicts a treeline in the 'Lautaret Valley' in the France-Alps
...
plotRGB(mspec,3,2,1)
```
The band combination (3,2,1) delivers the True Color Composit (TCC): a few trees with grass and bare soil.

```{r, message=FALSE,warning=FALSE}
plotRGB(mspec,4,3,2)
```

The band combination (4,3,2) delivers and IR Composite with the NIR band :-)
...
Depending on our dataset we can compute either 'RGB based indices' and/or (additionally) 'RGB+NIR based indices'. NOTE that if  'NIR' band is present in your dataset, 'RGB+NIR based indices' AND 'RGB based indices' should be compute to generate a LEGION of ALs.
...
Now lets see our results! Instead of the input data with 4 layers (the bands) we have additionally 15 ALs to 'feed' a machine learning algorithm (or other approach). So can we now go on to applying filter?
Yes, sure we could, BUT do you remember that indices looks quite the same? If we now compute filter, it could take a lot of time to compute all filter to ALL those indices. Propably most of this time would flow into filtering higly correlating layers to produce several (maybe hundreds) of additionally ALs which do not deliver additional informations.

So first thing now is to check for correlations! 

### Correaltion Check

The 'detect_RstCor' function of LEGION (who would've thought it!) will detect correlations within an input RasterStack and return a RasterStack only containing the least correlating RasterLayers depending on the predefined treshold value ('THvalue'). If the parameter "returnCorTab" is set on TRUE, it will return the correlation table instead of the RasterStack. 
...
The range of the 'THvalue' is the absolute corellation range from 0 to 1.0. 

Let's detect those correaltions!
```{r, message=FALSE,warning=FALSE}
# check for correlations
justcheck <- LEGION::detct_RstCor(rgbIND, 0.9)
```
In the case of the RasterStack with the RGB indices (rgbIND) a 'THvalue' of 0.9, leads to ditching every layer with cor > 0.9 and < -0.9, that is the NDTI, CI and NGRDI indices.
```{r, message=FALSE,warning=FALSE}
# check for correlations
justcheck2 <- LEGION::detct_RstCor(rgbNIR, 0.9)
```
In contrast, the RasterStack with the RGB+NIR indices (rgbNIR) with a THvalue = 0.9 only the NDVI and MSR indices, created from the RGB+NIR bands are dropped.
...
```
In this case the correlations differ a little and (in the case of THvalue=0.9) the filter with MovingWindof sized of 3*3 and 15*15 are kept.
...
Unlike with the indices we willnow select the filter functions by hand to compute them for the hole IndStk_clean RasterStack. But NOTE: like with the indices if you want to keep a layer which has been found to correlate feel free to keep it (because reasons)
...
```
Here we go: from just 4 bands of the original RasterStack (mspec) we now have 70 ALs with (for the indices proved) a correlation coefficient less than 0.9. So this should suit our machine laring (or other applications) well.
...
```
Lastly if we use the 'detct_RstCor' on our final RasterStack, exacly half of the RasterLayers are dropped. If we look at the correlationplot, we can see within the range of an indextype some of the filters correlate.

Its now up to you to decide to either keep the final Stack as it is or to reduce it.
But finally our goal was to check for correlation between the available indices and filter functions. So to keep or drop ALs is NOT content of this Tutorial or the LEGION package.

## Brute force approach

As mentioned in the beginning the 'fast' approach could lead a lost of infromations due to the selection of indices and filters based on their correlations. If time is not a big problem is t is possible to first compute all possible ALs and finally clean the Stack up from correlating Als. The code is quiet easy (but would take some time):

NOTE the code is outcommended. Feel free to test the 'brute force' approach 'overnight' or with specific indices only (to save some time).
...
By building a LEGION of ALs we receive a RasterStack with several different ALs to feed a machine learing algorithm (or other applications). The only desicion we made, was the 'THvalue' for the correlations and the filter sizes. So finally LEGION  helps to avoid 'overthinking' about decisions and delivers a workflow to generate a predictor RasterStack from just one input dataset in a automated comprehensible and justified statisical way.

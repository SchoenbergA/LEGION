...
If time is essential or with big input data it could be helpful to reduce the amount of ALs which have to be computed. In this case the correlations should be detected before further processing BUT it is possible to lose some informations.

This tutorial will lead you throught the 'fast' and comprehensible process of computing a RasterStack with several ALs with LEGION. Hereby we will start with just one input RasterLayer and 'form up a LEGION of ALs': the output RasterStack. Furthermore we will make sure that the output RasterStack is clean from highly correlating layers. (for the 'brute force approach' see respective chapter)
...
As mentioned in the beginning the 'fast' approach could lead to a lost of information due to the selection of indices and filters based on their correlations. It is possible that some of those ALs which have not been computed would lead to additional information and not be dropped in a final correlation detection. If time is not a big problem is t is possible to first compute all possible ALs and finally clean the Stack up from correlating Als. The code is quiet easy (but would take some time):
